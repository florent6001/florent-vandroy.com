...
export default function post({ post }) {
    return (
        <Link href={`/blog/${post.slug}`} key={post.slug} itemScope itemType="https://schema.org/BlogPosting">
          <a title={`Se rendre sur l'article "${post.frontmatter.title}"`} className="flex justify-between flex-col-reverse lg:flex-row lg:items-center py-5 text-white hover:bg-gray-800 bg-body px-5 transition duration-700 hover:no-underline">
